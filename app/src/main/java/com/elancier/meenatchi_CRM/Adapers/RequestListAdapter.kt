    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (viewType) {
            ITEM_CONTENT_VIEW_TYPE -> {
                ...
                holder.tours.setText(qty)

                holder.r_type.setText(type)
                holder.status.setText(status)

                if(from=="customer"){
                    holder.expenditure.setText(sub_total.toString())
                    holder.billing_approval.setText(discount.toString())
                    holder.account_approval.setText(total)
                    holder.sublay.visibility=View.VISIBLE
                    holder.totallay.visibility=View.VISIBLE
                    holder.r_amount.visibility=View.GONE
                    holder.orderlbl.setText("Invoice")

                }
                else{
                    holder.sublay.visibility=View.GONE
                    holder.totallay.visibility=View.GONE
                    holder.r_amount.visibility=View.VISIBLE

                }


                if(status=="Pending"||status=="Cancelled"){
                    holder.r_amount.setText(status.toString())
                    holder.r_amount.setTextColor(context.resources.getColor(R.color.red))

                }
                else{
                    holder.r_amount.setText(status.toString())
                    holder.r_amount.setTextColor(context.resources.getColor(R.color.paid))
