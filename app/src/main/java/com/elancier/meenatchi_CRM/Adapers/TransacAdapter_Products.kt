...
import com.elancier.meenatchi_CRM.*
import org.json.JSONArray

class TransacAdapter_Products(private var items: JSONArray, private val context: Activity) : RecyclerView.Adapter<RecyclerView.ViewHolder>(),Filterable
{
    ...
    override fun onCreateViewHolder(viewGroup: ViewGroup, viewType:Int): RecyclerView.ViewHolder {
        when (viewType) {
            ITEM_CONTENT_VIEW_TYPE -> {
                val productView = LayoutInflater.from(viewGroup.context).inflate(R.layout.transacsale_item, viewGroup, false)
        ...
        when (viewType) {
            ITEM_CONTENT_VIEW_TYPE -> {
               ...
               // val brand = if (obj.has("brand")) obj.getString("brand") else ""
                val amt = if (obj.has("amount")) obj.getString("amount") else ""
                val date = if (obj.has("date")) obj.getString("date") else ""
                val desc = if (obj.has("bill_no")) obj.getString("bill_no") else ""


                holder.brand.setText(date)
                holder.r_type.setText(amt)
                holder.r_desc.setText(desc)
