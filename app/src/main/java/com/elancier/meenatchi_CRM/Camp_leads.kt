...
import com.elancier.meenatchi_CRM.Adapers.PreviousCampAdap
...
class Camp_leads : AppCompatActivity(),PreviousCampAdap.OnItemClickListener,DatePickerDialog.OnDateSetListener  {
    val activity=this
    lateinit var pDialog : Dialog
    internal  var pref: SharedPreferences?=null
    internal  var editor: SharedPreferences.Editor?=null
    lateinit var adp : PreviousCampAdap
    var eodlists = ArrayList<EOD_data>()//JSONArray()
    lateinit var adpdup : PreviousCampAdap
    ...
    lateinit var wifenm:EditText
    lateinit var husnm:EditText
    lateinit var wifenum:EditText
    lateinit var husnum:EditText
    lateinit var wifeage:EditText
    lateinit var husage:EditText
    ...
    var crmattr=ArrayList<String>()
    var loclistIds=ArrayList<String>()

    var prefloclist=ArrayList<String>()
    var prefloclistIds=ArrayList<String>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adp = PreviousCampAdap(activity,eodlists,activity)
        camplist.adapter = adp

        adpdup = PreviousCampAdap(activity,eodlistsdup,activity)
        camplist.adapter = adpdup

        nodata.setText("No Camp Leads")
    ...
    fun getPrefLocations(){
        prefloclist.clear()
        prefloclistIds.clear()
        prefloclist.add("Select Location")
        prefloclistIds.add("0")
                    ...
                    if (response.isSuccessful()) {
                        ...
                        if (example.status == "Success") {
                            ...
                            for (r in 0 until example.response!!.size) {
                                ...
                                prefloclist.add(res.name.toString())
                                prefloclistIds.add(res.id.toString())
                                //eodlist.put(obj)
                            }
    ...
    fun addpopup() {
        ...
        val popUpView = layoutInflater.inflate(R.layout.addcamplist_popup, null)

        locationDropdown=popUpView.findViewById(R.id.campspinner);
        wifenm=popUpView.findViewById(R.id.wifenm);
        husnm=popUpView.findViewById(R.id.husnm);
        wifenum=popUpView.findViewById(R.id.wifenum);
        husnum=popUpView.findViewById(R.id.husnum);
        wifeage=popUpView.findViewById(R.id.wifeage);
        husage=popUpView.findViewById(R.id.husage);
        ...
        submit.setOnClickListener {
            if(locationDropdown.selectedItemPosition!=0&&wifenm.text.isNotEmpty()&&(wifenum.text.isNotEmpty()&&wifenum.text.length==10)
                &&husnm.text.isNotEmpty()&&(husnum.text.isNotEmpty()&&husnum.text.length==10)&&wifeage.text.isNotEmpty()&&husage.text.isNotEmpty()){
                saveCamp()
            }
            else{
                ...
                if(wifenm.text.isEmpty()){
                    wifenm.setError("Required field*")
                }
                if(wifenum.text.isEmpty()){
                    wifenum.setError("Required field*")
                }
                 if(wifeage.text.isEmpty()){
                     wifeage.setError("Required field*")
                }
                if(wifenum.text.length<10){
                    wifenum.setError("Invalid Mobile Number*")
                    Toast.makeText(applicationContext,"Invalid Mobile Number",Toast.LENGTH_SHORT).show()
                }
                if(husnum.text.length<10){
                    husnum.setError("Invalid Mobile Number*")
                    Toast.makeText(applicationContext,"Invalid Mobile Number",Toast.LENGTH_SHORT).show()
                }

                if(husnm.text.isEmpty()){
                    husnm.setError("Required field*")
                }
                if(husnum.text.isEmpty()){
                    husnum.setError("Required field*")
                }
                if(husage.text.isEmpty()){
                    husage.setError("Required field*")
                }
    ...
    fun saveCamp(){
        if (Appconstants.net_status(this)) {
            ...
            val call = ApproveUtils.Get.campleadstore(pref!!.getString("empid", "").toString(),loclistIds[locationDropdown.selectedItemPosition],wifenm.text.toString(),
                    husnm.text.toString(),wifenum.text.toString(),husnum.text.toString(),wifeage.text.toString(),
                    husage.text.toString())
            call.enqueue(object : Callback<Resp_otp> {
                override fun onResponse(call: Call<Resp_otp>, response: Response<Resp_otp>) {
                    Log.e("responce", response.toString())
                    if (response.isSuccessful()) {
    ...
    fun CampList(){
        ...
        if (Appconstants.net_status(this)) {
            ...
            val call = ApproveUtils.Get.getCampleads(json);
            call.enqueue(object : Callback<Resp_otp> {
                override fun onResponse(call: Call<Resp_otp>, response: Response<Resp_otp>) {
                    Log.e("responce", response.toString())
                    if (response.isSuccessful()) {
                        ...
                        if (example.status == "Success") {
                            ...
                            for (r in 0 until example.response!!.size) {
                                ...
                                for(i in 0 until loclistIds.size){
                                    try {
                                        if (res.camp == loclistIds[i]) {
                                            patient.cname = loclist[i]
                                            patient.mmob = crmattr[i]
                                ...
                                for(j in 0 until prefloclistIds.size){
                                    try {
                                        if (res.prefer_location == prefloclistIds[j]) {

                                            patient.tval= prefloclist[j]
                                ...
                                patient.maddress = res.wife_name
                                patient.city = res.husband_name
                                patient.email = res.wife_no
                                patient.state = res.husband_no
    ...
    fun getLocations(){
        ...
        loclistIds.clear()
        crmattr.clear()
        loclist.add("Select Camp")
        crmattr.add("Select Camp")
        loclistIds.add("0")
        if (Appconstants.net_status(this)) {
            //getEOD().execute()
            /*  val objs = JsonObject()
            ...
            var json=JsonObject()
            json.addProperty("user_id",pref!!.getString("empid", "").toString())
            val call = ApproveUtils.Get.getcamps(json)
            call.enqueue(object : Callback<Resp_otp> {
                override fun onResponse(call: Call<Resp_otp>, response: Response<Resp_otp>) {
                    Log.e("responce", response.toString())
                    if (response.isSuccessful()) {
                        ...
                        if (example.status == "Success") {
                            ...
                            for (r in 0 until example.response!!.size) {
                                ...
                                val res = example.response!!.get(r)
                                loclist.add(res.camp_location.toString())
                                crmattr.add(res.crm_att.toString())
                ...
                if(newText!!.length>=3) {
                    ...
                    if (eodlists.isNotEmpty()) {
                        ...
                        for (i in 0 until eodlists.size) {
                            ...
                            try {
                                if (eodlists[i].cname!!.toString().toLowerCase()
                                        .contains(newText!!.toLowerCase()) ||
                                    eodlists[i].mmob!!.toString().toLowerCase()
                                        .contains(newText!!.toLowerCase()) ||
                                    eodlists[i].tval!!.toString().toLowerCase()
                                        .contains(newText!!.toLowerCase()) ||
                                    eodlists[i].maddress!!.toString().toLowerCase()
                                        .contains(newText!!.toLowerCase()) ||
                                    eodlists[i].city!!.toString().toLowerCase()
                                        .contains(newText!!.toLowerCase()) ||
                                    eodlists[i].email!!.toString().toLowerCase()
                                        .contains(newText!!.toLowerCase()) ||
                                    eodlists[i].state!!.toString().toLowerCase()
                                        .contains(newText!!.toLowerCase())
                                ) {
                                    val data = EOD_data()
                                    ...
                                    data.maddress = eodlists[i].maddress.toString()
                                    data.city = eodlists[i].city.toString()
                                    data.email = eodlists[i].email.toString()
                                    data.state = eodlists[i].state.toString()
                                    eodlistsdup.add(data)


                                }
                                adpdup = PreviousCampAdap(
                                    this@Camp_leads, eodlistsdup, activity
                                )
                                camplist.adapter = adpdup
