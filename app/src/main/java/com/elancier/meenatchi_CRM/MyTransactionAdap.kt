    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (viewType) {
            ITEM_CONTENT_VIEW_TYPE -> {
                ...
                val status = if (obj.has("paid_amount")) obj.getString("paid_amount") else ""
                val qty = if (obj.has("date")) obj.getString("date") else ""

                holder.r_type.setText(qty)
                holder.tours.setText(type)
                holder.expenditure.setText(status)

                val expenditure_plan = if (obj.has("details")) obj.getJSONArray("details") else JSONArray()
