...
class Request_Add:AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (frm == "customer") {
            ab!!.title = "Add Business"
        } else {
            ab!!.title = "Add Expense"
            tt.setHint("Expense Name")
    ...
    fun SendLogin(mobile:String,url:String,url1:String){
        ...
        if(frm=="customer"){
            val json=JSONObject()
            json.put("tid",pref.getString("tid","").toString())
            json.put("user",pref.getString("mobile","").toString())
            json.put("request_type",idarr[cityspin.selectedItemPosition])
            json.put("name",fname!!.text.toString())
            json.put("description",address!!.text.toString())
            json.put("image","data:image/png;base64"+","+imagecode)
            json.put("image1","")
            json.put("amount",mob.text.toString().trim())

            println("NEW BUSINESS"+json.toString())
            call = ApproveUtils.Get.getreqentry(pref.getString("tid","").toString(),
                pref.getString("mobile","").toString(),idarr[cityspin.selectedItemPosition] ,fname!!.text.toString(),address!!.text.toString(),
                    "data:image/png;base64"+","+imagecode,"",mob.text.toString().trim())
        }
        else{
    ...
    fun types(){
        if (Appconstants.net_status(this)) {
            ...
            if(frm=="customer"){
                call = ApproveUtils.Get.getreqtype()
            }
            else{
                    ...
                    if (response.isSuccessful()) {
                        ...
                        if (example.getStatus() == "Success") {
                          ...
                          //  textView23.visibility= View.GONE
                            ...
                            val type=otpval[i].type.toString()
                            nmarr.add(type)
                            idarr.add(id)
...
//                            textView23.visibility= View.GONE
                            ...
                            Toast.makeText(this@Request_Add, example.status, Toast.LENGTH_SHORT)
                                .show()
                        }
                    }
                    else{
